...
insert into NhanVien (MaNV,HoTen,GioiTinh,DiaChi,NgaySinh,DienThoai,Email,NoiSinh,NgayVaoLam) values
("NV11","Nhan Vien 11",1,"Ha Noi","19960427","111111110","nhanvien11@gmail.com");
-- 3. UPDATE 
-- Cập nhật lại số điện thoại mới cho khách hàng mã KH10.
update KhachHang set SoDT = "101010101" where id = "KH10";
-- Cập nhật lại địa chỉ mới của nhân viên mã NV05.
update NhanVien set DiaChi = "HCM" where id = "NV05";
-- 4. DELETE
-- Xóa nhân viên mới vừa thêm ở 2.3
delete from NhanVien where id = "NV11";
-- Xóa sản phẩm mã SP15
delete from SanPham where id = "SP15";
-- 5. SELECT
...
select nv.MaNV `Ma_Nhan_Vien`, nv.HoTen `Ho_Ten`, nv.GioiTinh `Gioi_Tinh`, nv.NgaySinh `Ngay_Sinh`, 
nv.DiaChi `Dia_Chi`, nv.DienThoai `SDT`, timestampdiff(YEAR, nv.NgaySinh, CURDATE()) as Age  
from NhanVien nv order by age;
-- 2. Liệt kê các hóa đơn nhập hàng trong tháng 6/2023, 
...
select * from SanPham sp where sp.Donvitinh = "Chai";
